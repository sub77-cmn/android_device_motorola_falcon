...
patchdir="$build_root/hardware/qcom/display" # Start patch1
echo -e ${CL_BLU}"PATCHING $patchdir"${CL_RST}
cd $patchdir
git fetch http://gerrit.dirtyunicorns.com/android_hardware_qcom_display refs/changes/28/2028/1 && git cherry-pick FETCH_HEAD
cd $build_root # End patch1

patchdir="$build_root/hardware/qcom/keymaster" # Start patch1
echo -e ${CL_BLU}"PATCHING $patchdir"${CL_RST}
cd $patchdir
git fetch http://gerrit.dirtyunicorns.com/android_hardware_qcom_keymaster refs/changes/30/2030/1 && git cherry-pick FETCH_HEAD
cd $build_root # End patch1

patchdir="$build_root/hardware/qcom/keymaster" # Start patch1
echo -e ${CL_BLU}"PATCHING $patchdir"${CL_RST}
cd $patchdir
git fetch http://gerrit.dirtyunicorns.com/android_hardware_qcom_keymaster refs/changes/31/2031/1 && git cherry-pick FETCH_HEAD
cd $build_root # End patch1
